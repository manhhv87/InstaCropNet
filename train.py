...
def train():
    ...
    val_dataset = TusimpleSet(
        val_dataset_file, transform=data_transforms['val'], target_transform=target_transforms)
    ...
    dataset_sizes = {'train': len(
        train_loader.dataset), 'val': len(val_loader.dataset)}
    ...
    model, log = train_model(model, optimizer, scheduler=None, dataloaders=dataloaders,
                             dataset_sizes=dataset_sizes, device=DEVICE, loss_type=args.loss_type, num_epochs=args.epochs)
    df = pd.DataFrame({'epoch': [], 'training_loss': [],
                      'val_loss': [], 'binary_loss': [], 'instance_loss': []})
    df['epoch'] = log['epoch']
    ...
    df.to_csv(train_log_save_filename, columns=[
              'epoch', 'training_loss', 'val_loss', 'binary_loss', 'instance_loss'], header=True, index=False, encoding='utf-8')
