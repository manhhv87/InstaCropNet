...
class _SegCropNetCluster(object):
    ...
    def apply_lane_feats_cluster(self, binary_seg_result, instance_seg_result):
        ...
        mask = np.zeros(shape=[binary_seg_result.shape[0],
                        binary_seg_result.shape[1], 3], dtype=np.uint8)
    ...
    def postprocess(self, binary_seg_result, instance_seg_result=None,
        ...
        if not with_lane_fit:
            ...
            source_image = cv2.addWeighted(
                source_image, 0.6, tmp_mask, 0.4, 0.0, dst=source_image)
            return {
        ...
        for lane_index, coords in enumerate(lane_coords):
            ...
            fit_x = fit_param[0] * plot_y ** 2 + \
                fit_param[1] * plot_y + fit_param[2]
        ...
        for index, single_lane_pts in enumerate(src_lane_pts):
            single_lane_pt_x = np.array(
                single_lane_pts, dtype=np.float32)[:, 0]
            single_lane_pt_y = np.array(
                single_lane_pts, dtype=np.float32)[:, 1]
            if data_source == 'corn':
                start_plot_y = 240
                end_plot_y = 720
            else:
            ...
            for plot_y in np.linspace(start_plot_y, end_plot_y, step):
                ...
                interpolation_src_pt_x = (abs(previous_src_pt_y - plot_y) * previous_src_pt_x +
                                         ...
                                         (abs(previous_src_pt_y - plot_y) +
                                          abs(last_src_pt_y - plot_y))
