...
def adjust_centers_order(centers_1, centers_2, centers_3, centers_4):
    ...
    # Adjust the order of the four arrays centers_1, centers_2, centers_3,
    # and centers_4, and temporarily save the adjusted order to the centers_mid array
    centers_mid = []
...
def lane_detection(binary_image_path, instance_image_path, num_clusters, min_cluster_size=100):
    ...
    # Use jet colormap to show different instances
...
def plot_data_and_line(image, data_points, line_width=1.0):
    ...
    for i, (x, y) in enumerate(data_points):
        ...
        if np.isinf(m):
            ...
            plt.plot(
                x, m * x + c, color=colors[i], label=f'Line {i + 1}', linewidth=line_width)
